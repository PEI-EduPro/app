...
- Make the warnings table with the following fields: id (internal autogenerated), type (enum with the multiple_students_to_exam, or multiple_exams_to_student, or exam_correction_no_student), student list (string with a tuple nmec:name) and exam list (ids of the multiple exams).
- Delete the "finished" state in the waiting room.
